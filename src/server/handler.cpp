...
SocketHandler::~SocketHandler()
{
    ...
    for (int i = 0; i < _num_sockets; ++i)
        close(_pollfds[i].fd);
    for (std::map<int, ClientRequestState>::iterator it = _conns.begin(); it != _conns.end(); ++it)
        close(it->first);
}
...
void SocketHandler::processData(int i)
{
    ...
    _conns[_pollfds[i].fd].expectedSize = -1;
    _conns[_pollfds[i].fd].keepalive = 0;
    _conns[_pollfds[i].fd].server = NULL;
