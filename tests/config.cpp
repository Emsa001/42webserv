...
// Test that valid configs with extreme values work correctly
TEST_F(ConfigTest, StressTestExtremeValidValues) {
    EXPECT_TRUE(testConfigFile("tests/valid_configs/extreme_values.yml"));
    ...
    if (servers.size() > 0) {
        config_map server = servers[0].getMap();
        // Test that extreme values are parsed correctly
        EXPECT_GT(server["max_client_body_size"].getInt(), 0);
        EXPECT_GT(server["max_client_header_size"].getInt(), 0);
    }
}

// Test performance with large configuration file
TEST_F(ConfigTest, StressTestPerformance) {
    EXPECT_TRUE(testConfigFile("tests/valid_configs/performance_stress.yml"));
...
// Test configuration with many different location configurations
TEST_F(ConfigTest, StressTestComplexLocations) {
    EXPECT_TRUE(testConfigFile("tests/valid_configs/full_featured.yml"));
    ...
    if (servers.size() > 0) {
        config_map server = servers[0].getMap();
        if (server.find("locations") != server.end()) {
            config_array locations = server["locations"].getArray();
